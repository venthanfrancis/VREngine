...
**M0 through M6 complete.** `Core` (math: Vec2/Vec3/Vec4/Quaternion
...
hierarchy/`GetWorldMatrix`), and `Assets` (`AssetId`/`TextAsset`/
`BinaryAsset`/`AssetManager` with an asset root, caching, and
root-traversal protection) are all implemented. `AREngineSandbox.exe`
opens a window, runs a real frame loop, submits one hard-coded temporary
dummy draw per frame through the Null backend (nothing visibly renders —
that's expected, see `docs/ARCHITECTURE.md` Section 12), logs FPS once
per second, and shuts down cleanly on close. Raw keyboard/mouse input
and file I/O via `Platform` were deliberately NOT added in M2 — see
Section 10 (`Assets` reads files itself, via `std::filesystem`/
`std::ifstream` directly, not through `Platform` — see Section 14).
**Neither `Scene` nor `Assets` is wired into `Runtime` yet** — nothing
consumes either of them yet (no `Scene`→`Rendering` bridge, no asset
content parsers), so both are tested entirely headlessly; see Sections
13 and 14 for why that's the correct scope, not a gap. There is still no
real graphics backend, no frame limiting, no pipeline/shader API
(deferred until Vulkan, M8, per Section 12), no ECS/component system
(Section 13), and no real mesh/texture/image format parsing (Section
14) — `DesktopFrameDriver` produces one placeholder identity `ViewInfo`
per frame and `SubmitFrame()` is a no-op — see Section 11. Everything
else (`Input`, `XR`, `Editor`) is still an M0-style stub with no
functionality. Next up is M7 (`Input`). See `docs/ROADMAP.md` for the
full plan.
