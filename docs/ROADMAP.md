...
| — M11.3 | **XR teardown/lifetime investigation: PARTIAL / BLOCKED.** Completed: explicit XR/Vulkan ownership and destruction graph; teardown instrumentation added to XR demos; runtime-vs-AREngine Vulkan handle classification; OpenXR lifetime/order audit; identified missing `vkDeviceWaitIdle` consistency in `openxr_frame_demo`. Still pending: live instrumented Meta reproduction; exact failing teardown call/thread; Meta crash/log analysis; official `hello_xr` Vulkan comparison; final AREngine vs runtime vs interop root-cause classification. | **Partial / Blocked** | Blocker: Windows Smart App Control preventing required locally-built test executables from running - see `docs/ARCHITECTURE.md`, "M11.3 - Teardown Investigation". M11.1 Steps 5-10 remain blocked until this resolves. |
| — M11.3A | **Smart App Control diagnosis: COMPLETE.** Findings: Smart App Control enforcement confirmed through Code Integrity events; affected binaries were unsigned local compiler outputs with no Mark-of-the-Web; the differentiator was hash/execution history under SAC enforcement, not file origin, ACLs, or MOTW; clean relink produced a new hash and restored execution; full CTest returned to 17/17 with Smart App Control still enabled; no Defender/SmartScreen/firewall/security weakening required. | **Complete** | Development rule: if a locally-built test executable is unexpectedly blocked under SAC, relink/rebuild it before changing Windows security settings. See `docs/ARCHITECTURE.md`, "M11.3A - Smart App Control Diagnosis". M11.3's own XR teardown investigation remains blocked by an unrelated SteamVR runtime issue. |
