...
| M18 | Physics Foundation: fixed stepping, Jolt bodies, explicit Scene bridge | **Complete** | Headless collision/lifetime tests and Vulkan demo validated; see architecture M18 |
| M19 | Audio Foundation: cached WAV content, reusable clips/sources, one listener, spatial playback | **Complete with external limitation** | Offline signal tests and physical-device lifecycle pass; perceptual listening remains unconfirmed and does not block foundation closure |
